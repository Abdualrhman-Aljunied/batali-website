...
async function uploadFileToDrive(file, orderNumber) {
  ...
  if (!allowedTypes.includes(file.type)) {
    throw new Error('يسمح فقط برفع صور JPG أو PNG أو WEBP أو ملفات PDF.');
  }

  // الحد الأقصى 10MB
  if (file.size > 10 * 1024 * 1024) {
    throw new Error('الحد الأقصى لحجم الملف هو 10 ميجابايت.');
  }
  ...
  try {
    ...
    if (err.name === 'AbortError')
      throw new Error('انتهت مهلة الرفع (30 ثانية) — تحقّق من اتصال الإنترنت وحاول مجدداً');
